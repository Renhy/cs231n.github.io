    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            if self.normalization=='batchnorm':
                ...
                hidden, cache = affine_bn_relu_forward(hidden, W, b, gamma, beta, self.bn_params[i - 1])
            else:
                hidden, cache = affine_relu_forward(hidden, W, b)
            hiddens.append(hidden)
            caches.append(cache)
        
        scores, scores_cache = affine_forward(hidden, self.params['W' + str(self.num_layers)], self.params['b' + str(self.num_layers)])
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        for i in range(self.num_layers):
            re += np.sum(self.params['W' + str(i + 1)] ** 2)
        loss += 0.5 * self.reg * re
        
        dhidden, dWL, dbL = affine_backward(dscore, scores_cache)
        grads['W' + str(self.num_layers)] = dWL + self.reg * self.params['W' + str(self.num_layers)]
        grads['b' + str(self.num_layers)] = dbL
        
        for i in range(self.num_layers - 1, 0, -1):
            if self.normalization=='batchnorm':
                dhidden, dW, db, dgamma, dbeta = affine_bn_relu_backward(dhidden, caches[i - 1])
